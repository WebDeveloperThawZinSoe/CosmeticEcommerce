...
<div class="page-content">
    <!-- Banner Section -->
    <div class="dz-bnr-inr style-1" style="background-image: url('{{ $ProductCategory_detail->icon ? asset('images/product_categories/' . $ProductCategory_detail->icon) : asset('web/images/background/bg-shape.jpg') }}'); background-size: cover; background-position: center; min-height: 400px; display: flex; align-items: center;">
        <div class="container">
            <div class="dz-bnr-inr-entry text-center">
                ...
                <nav aria-label="breadcrumb" class="breadcrumb-row">
                    <ul class="breadcrumb d-inline-block" style="background-color: rgba(0, 0, 0, 0.6); padding: 10px; border-radius: 5px;">
                                            ...
                                            <h6 class="price" style="color:black !important;">
                                                @if($product->product_type == 1)
                                                    ...
                                                    @elseif($product->discount_type == 1)
                                                        @php $discount_price = $product->price - $product->discount_amount; @endphp
                                                        <del>{{ $product->price }}</del> {{ $discount_price }} {{ $product->country === 'myanmar' ? 'Ks' : ($product->country === 'korea' ? '₩' : '$') }}
                                                    @elseif($product->discount_type == 2)
                                                        @php $discount_price = $product->price - ($product->price * ($product->discount_amount / 100)); @endphp
                                                        <del>{{ $product->price }}</del> {{ $discount_price }} {{ $product->country === 'myanmar' ? 'Ks' : ($product->country === 'korea' ? '₩' : '$') }}
                                                    @endif
                                        ...
                                        <div class="product-tag">
                                            <span class="badge badge-secondary">Sale |
                                                @if($product->discount_type == 1)
                                                    {{ $product->discount_amount }} {{ $product->country === 'myanmar' ? 'Ks' : ($product->country === 'korea' ? '₩' : '$') }} OFF
                                                @elseif($product->discount_type == 2)
                        ...
                        <div class="col-md-6">
                            <nav aria-label="Blog Pagination">
                                <ul class="pagination style-1">
                                    ...
                                    @else
                                        <li class="page-item"><a class="page-link prev" href="{{ $products->previousPageUrl() }}">Prev</a></li>
                                    @endif

                                    @foreach($products->links()->elements[0] as $page => $url)
                                        <li class="page-item {{ $page == $products->currentPage() ? 'active' : '' }}">
                                            @if($page == $products->currentPage())
                                                <span class="page-link" style="background-color:black !important;color:white !important;">{{ $page }}</span>
                                            @else
